...
def nested_sampler_hmc(
    ...
    def _nested_sampling_itr(carry, itr):
        ...
        sample_points = NSPointSet(
            ...
            logwlike=ns_data.sample_points.logwlike.at[ns_data.n_iter].set(
                dead_logwlike
            ),
        )
        ...
        min_loglike_ind, next_min_loglike_ind = jax.lax.cond(
            live_points.loglike[min_loglike_ind]
            < live_points.loglike[next_min_loglike_ind],
            lambda x0, x1: (x0, x1),
    ...
    def _accumulate_correction(carry, dead_index):
        ...
        ns_data = NSData(
            ...
            live_points=ns_data.live_points,
            sample_points=NSPointSet(
                theta=ns_data.sample_points.theta.at[ns_data.n_iter + dead_index].set(
                    dead_theta
                ),
                loglike=ns_data.sample_points.loglike.at[
                    ns_data.n_iter + dead_index
                ].set(dead_loglike),
                logwlike=ns_data.sample_points.logwlike.at[
                    ns_data.n_iter + dead_index
                ].set(dead_logwlike),
            ),
            rng_key=ns_data.rng_key,
    ...
    ns_data = _ns_data_set_logw(
        ns_data, -ns_data.n_iter / ns_data.n_live - jnp.log(ns_data.n_live)
    )
    ns_data, _ = jax.lax.scan(
